...
if(MODE == mode.DEBUG): part_size *= 12
...
fluid_cube_data_1 = Cube_data(type=Cube_data.FIXED_CELL_SIZE, lb=vec2f(-4+3*span, -5+4*span), rt=vec2f(4-3*span, 3), span=span)
...
box_data   = Box_data(lb=vec2f(-4+3*span, -6.0), rt=vec2f(4, 4), span=world.getPartSize()*1.001, layers=3)
rod_data_1 = Box_data(lb=vec2f(-3.5, -5.8), rt=vec2f(-2.5, -5.0), span=world.getPartSize()*1.001, layers=3)
rod_data_2 = Box_data(lb=vec2f(-2.0, -5.8), rt=vec2f(-1.0, -5.0), span=world.getPartSize()*1.001, layers=3)
rod_data_3 = Box_data(lb=vec2f(-0.5, -5.8), rt=vec2f( 1.5, -5.0), span=world.getPartSize()*1.001, layers=3)
...
def loop(is_log_loop=False):
    ...
    rod_part_1.getSolverAdv().add_acc_harmonic(axis=1, dir = -1, period=0.5, amplitude=0.45, world_time=world.getTime(), delay=0)
    rod_part_2.getSolverAdv().add_acc_harmonic(axis=1, dir = -1, period=1.0, amplitude=0.45, world_time=world.getTime(), delay=0)
    rod_part_3.getSolverAdv().add_acc_harmonic(axis=1, dir = -1, period=0.7, amplitude=0.45, world_time=world.getTime(), delay=0)
...
def run(loop):
    ...
    gui2d_part = Gui2d(objs=[fluid_part, bound_part, rod_part_1, rod_part_2, rod_part_3], radius=world.getPartSize(), lb=vec2f([-8,-8]),rt=vec2f([8,8]),dpi=dpi)
