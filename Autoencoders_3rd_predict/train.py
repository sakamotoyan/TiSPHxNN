...
autoencoder.to('cuda')
if os.path.exists(os.path.join(model_path,AEname)):
    autoencoder.load_state_dict(torch.load(os.path.join(model_path,AEname)))
...
for epoch in range(num_epochs):
    ...
    for batch_inputs, batch_targets in data_loader:
        ...
        for i in range(num_forecast_steps): 
            targets.append(batch_targets[i].to('cuda'))
...
torch.save(autoencoder.state_dict(), os.path.join(model_path,AEname))