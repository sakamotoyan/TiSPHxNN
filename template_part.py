...
def part_template(part_obj, world, verbose=False):
    ...
    part_obj.add_array("vel_adv", vecxf(part_obj.get_world().g_dim[None]).field())
    ...
    part_obj.add_array("acc", vecxf(part_obj.get_world().g_dim[None]).field())
    part_obj.add_array("rgb", vecxf(3).field())
    
    part_obj.add_attr("statistics_linear_momentum", vecx_f(part_obj.get_world().g_dim[None]))
    ...
    # val_frac=ti.f32,
    # phase_vel=vecxf(part_obj.get_world().g_dim[None]),
    # phase_acc=vecxf(part_obj.get_world().g_dim[None]),
    # phase_force=vecxf(part_obj.get_world().g_dim[None]),
    # )
    ...
    sph_df = ti.types.struct(
        alpha_1=vecxf(part_obj.get_world().g_dim[None]),
        alpha_2=ti.f32,
        alpha=ti.f32,
        kappa_incomp=ti.f32,
        kappa_div=ti.f32,
        ...
        vel_adv=vecxf(part_obj.get_world().g_dim[None]),
    )
    ...
    phase = ti.types.struct(
        ...
        val_frac_in=ti.f32,
        val_frac_out=ti.f32,
        vel=vecxf(part_obj.get_world().g_dim[None]),
        drift_vel=vecxf(part_obj.get_world().g_dim[None]),
        acc=vecxf(part_obj.get_world().g_dim[None]),
    )
    mixture = ti.types.struct(
        lamb = ti.f32,
        flag_negative_val_frac = ti.i32,
        acc_pressure=vecxf(part_obj.get_world().g_dim[None]),
    )
