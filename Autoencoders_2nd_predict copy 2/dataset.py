...
class CustomDataset(Dataset):
    ...
    def __len__(self):
        return len(self.data) - 1  # subtract 1 to avoid index error

    def __getitem__(self, idx):
        sample_current = self.data[idx]  # current frame
        sample_next = self.data[idx + 1]  # next frame
        return sample_current, sample_next  # returns current frame and next frame
