    ...
    for part_obj in self.wcsph_solver_list:
        if part_obj.m_is_dynamic is True:
            for neighb_obj in part_obj.get_module_neighbSearch().neighb_obj_list:
                part_obj.get_module_neighbSearch().loop_neighb(neighb_obj, part_obj.getSolverWCSPH().inloop_add_acc_pressure)
...
def step_wcsph_add_acc_number_density_pressure(self):
    ...
    for part_obj in self.wcsph_solver_list:
        if part_obj.m_is_dynamic is True:
            for neighb_obj in part_obj.get_module_neighbSearch().neighb_obj_list:
                part_obj.get_module_neighbSearch().loop_neighb(neighb_obj, part_obj.getSolverWCSPH().inloop_add_acc_number_density_pressure)