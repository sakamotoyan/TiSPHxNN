...
rod_data_1 = Box_data(lb=vec2f(-5.5, 0), rt=vec2f(-5, 3), span=world.getPartSize()*1.001, layers=3)
rod_data_2 = Box_data(lb=vec2f(-5.5, -3), rt=vec2f(-5, -1), span=world.getPartSize()*1.001, layers=3)
...
def loop(is_log_loop=False):
    ...
    rod_part_1.getSolverAdv().add_acc_harmonic(axis=0, dir = -1, period=2, amplitude=0.5, world_time=world.getTime(), delay=0.5)
    rod_part_2.getSolverAdv().add_acc_harmonic(axis=0, dir = -1, period=2, amplitude=0.5, world_time=world.getTime(), delay=0)
...
def run(loop):
    ...
    gui2d_part = Gui2d(objs=[fluid_part, bound_part, rod_part_1, rod_part_2], radius=world.getPartSize(), lb=vec2f([-8,-8]),rt=vec2f([8,8]),dpi=dpi)
